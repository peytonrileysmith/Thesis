...
#Trace plots
lasso_model <- glmnet(X, data_output$y, alpha = 1)
pdf("lasso_trace_plot.pdf", width = 10, height = 6)
plot(lasso_model, xvar = "lambda", label = TRUE)
dev.off()

sim_lasso_model <- glmnet(X_sim, data_output$y, alpha = 1)
pdf("sim_lasso_trace_plot.pdf", width = 10, height = 6)
plot(sim_lasso_model, xvar = "lambda", label = TRUE)
dev.off()
...
N_Lasso_cor <- length(p_vals_cor_Lasso)
N_Ridge_cor <- length(p_vals_cor_Ridge)
N_EN_cor <- length(p_vals_cor_EN)

# Bonferroni corrected thresholds
alpha_bonf_Lasso_cor <- 0.05 / N_Lasso_cor
alpha_bonf_Ridge_cor <- 0.05 / N_Ridge_cor
alpha_bonf_EN_cor <- 0.05 / N_EN_cor

# Count significant p-values after Bonferroni correction
sig_Lasso_cor <- (sum(p_vals_cor_Lasso < alpha_bonf_Lasso_cor))/(length(p_vals_cor_Lasso))*100
sig_Ridge_cor <- (sum(p_vals_cor_Ridge < alpha_bonf_Ridge_cor))/(length(p_vals_cor_Ridge))*100
sig_EN_cor <- (sum(p_vals_cor_EN < alpha_bonf_EN_cor))/(length(p_vals_cor_EN))*100

#Create tables for full cohort and save
correlation_table <- data.frame(
  Metric = c("Proportion of Significant P-Values (<0.05)"),
  LASSO = c(sprintf("%.1f%%", sig_Lasso_cor)),
  Ridge = c(sprintf("%.1f%%", sig_Ridge_cor)),
  Elastic_Net = c(sprintf("%.1f%%", sig_EN_cor))
)

# Create the HTML table for correlation
correlation_table_html <- kable(correlation_table, format = "html", 
                                col.names = c("Metric", "LASSO", "Ridge", "Elastic Net")) %>%
  kable_styling(full_width = F, position = "center") %>%
  add_header_above(c("Full Sample Z-Test Results (Correlation)" = 4), bold = TRUE)

# Save the correlation HTML table to a file
correlation_html_file <- "fullsampletable_correlation_spearman.html"
save_kable(correlation_table_html, file = correlation_html_file)

# Verify the HTML file is created
print(paste("HTML file for correlation table saved as:", correlation_html_file))

#Change column names
#AUC
colnames(df_auc_Lasso_sim) <- c("s_vector", "auc_vector_Lasso")
colnames(df_auc_EN_sim)<-c("s_vector", "auc_vector_EN")
colnames(df_auc_Ridge_sim)<-c("s_vector", "auc_vector_Ridge")
#Correlation
colnames(df_cor_Lasso_sim) <- c("s_vector", "cor_vector_Lasso")
colnames(df_cor_EN_sim)<-c("s_vector", "cor_vector_EN")
colnames(df_cor_Ridge_sim)<-c("s_vector", "cor_vector_Ridge")


# Combine the data frames for LASSO without and with simulated data 
df_cor_Lasso_combined <- rbind(
  cbind(df_cor_Lasso, Simulation = "No Simulated Metabolites"),
  cbind(df_cor_Lasso_sim, Simulation = "Including Simulated Metabolites")
)

# Plotting Correlation for LASSO
corplot_lasso <- ggplot() +
  geom_point(data = df_cor_Lasso_combined, aes(x = s_vector, y = cor_vector_Lasso, color = Simulation), size = 2) +
  labs(x = "Training Set Size", y = "Correlation",
       title = "Correlation Analysis on LASSO Regression with and without Simulated Metabolites") +
  scale_color_manual(values = c("No Simulated Metabolites" = "blue", "Including Simulated Metabolites" = "red")) +
  theme_minimal() +
  theme(plot.title = element_text(size = 14), legend.title = element_text(size = 12), legend.text = element_text(size = 10))

# Save the plot to a file
ggsave(filename = "lasso_correlation_plot.png", plot = corplot_lasso, width = 8, height = 6, dpi = 300)


# Combine the data frames for Ridge without and with simulated data
df_cor_Ridge_combined <- rbind(
  cbind(df_cor_Ridge, Simulation = "No Simulated Metabolites"),
  cbind(df_cor_Ridge_sim, Simulation = "Including Simulated Metabolites")
)

# Plotting Correlation for Ridge
corplot_ridge <- ggplot() +
  geom_point(data = df_cor_Ridge_combined, aes(x = s_vector, y = cor_vector_Ridge, color = Simulation), size = 2) +
  labs(x = "Training Set Size", y = "Correlation",
       title = "Correlation Analysis on Ridge Regression with and without Simulated Metabolites") +
  scale_color_manual(values = c("No Simulated Metabolites" = "blue", "Including Simulated Metabolites" = "red")) +
  theme_minimal() +
  theme(plot.title = element_text(size = 14), legend.title = element_text(size = 12), legend.text = element_text(size = 10))

# Save the plot to a file
ggsave(filename = "ridge_correlation_plot.png", plot = corplot_ridge, width = 8, height = 6, dpi = 300)

# Combine the data frames for Elastic Net without and with simulated data
df_cor_EN_combined <- rbind(
  cbind(df_cor_EN, Simulation = "No Simulated Metabolites"),
  cbind(df_cor_EN_sim, Simulation = "Including Simulated Metabolites")
)

# Plotting Correlation for Elastic Net
corplot_en <- ggplot() +
  geom_point(data = df_cor_EN_combined, aes(x = s_vector, y = cor_vector_EN, color = Simulation), size = 2) +
  labs(x = "Training Set Size", y = "Correlation",
       title = "Correlation Analysis on Elastic Net Regression with and without Simulated Metabolites") +
  scale_color_manual(values = c("No Simulated Metabolites" = "blue", "Including Simulated Metabolites" = "red")) +
  theme_minimal() +
  theme(plot.title = element_text(size = 14), legend.title = element_text(size = 12), legend.text = element_text(size = 10))

# Save the plot to a file
ggsave(filename = "elastic_net_correlation_plot.png", plot = corplot_en, width = 8, height = 6, dpi = 300)

# Combine the data frames for LASSO without and with simulated data
df_auc_Lasso_combined <- rbind(
  cbind(df_auc_Lasso, Simulation = "No Simulated Metabolites"),
  cbind(df_auc_Lasso_sim, Simulation = "Including Simulated Metabolites")
)

# Plotting AUC for LASSO
aucplot_lasso <- ggplot() +
  geom_point(data = df_auc_Lasso_combined, aes(x = s_vector, y = auc_vector_Lasso, color = Simulation), size = 2) +
  labs(x = "Training Set Size", y = "AUC Value",
       title = "AUC Analysis on LASSO Regression with and without Simulated Metabolites") +
  scale_color_manual(values = c("No Simulated Metabolites" = "blue", "Including Simulated Metabolites" = "red")) +
  theme_minimal() +
  theme(plot.title = element_text(size = 14), legend.title = element_text(size = 12), legend.text = element_text(size = 10))

# Save the plot to a file
ggsave(filename = "lasso_auc_plot.png", plot = aucplot_lasso, width = 8, height = 6, dpi = 300)

# Combine the data frames for Ridge without and with simulated data
df_auc_Ridge_combined <- rbind(
  cbind(df_auc_Ridge, Simulation = "No Simulated Metabolites"),
  cbind(df_auc_Ridge_sim, Simulation = "Including Simulated Metabolites")
)

# Plotting AUC for Ridge
aucplot_ridge <- ggplot() +
  geom_point(data = df_auc_Ridge_combined, aes(x = s_vector, y = auc_vector_Ridge, color = Simulation), size = 2) +
  labs(x = "Training Set Size", y = "AUC Value",
       title = "AUC Analysis on Ridge Regression with and without Simulated Metabolites") +
  scale_color_manual(values = c("No Simulated Metabolites" = "blue", "Including Simulated Metabolites" = "red")) +
  theme_minimal() +
  theme(plot.title = element_text(size = 14), legend.title = element_text(size = 12), legend.text = element_text(size = 10))

# Save the plot to a file
ggsave(filename = "ridge_auc_plot.png", plot = aucplot_ridge, width = 8, height = 6, dpi = 300)

# Combine the data frames for Elastic Net without and with simulated data
df_auc_EN_combined <- rbind(
  cbind(df_auc_EN, Simulation = "No Simulated Metabolites"),
  cbind(df_auc_EN_sim, Simulation = "Including Simulated Metabolites")
)

# Plotting AUC for Elastic Net
aucplot_en <- ggplot() +
  geom_point(data = df_auc_EN_combined, aes(x = s_vector, y = auc_vector_EN, color = Simulation), size = 2) +
  labs(x = "Training Set Size", y = "AUC Value",
       title = "AUC Analysis on Elastic Net Regression with and without Simulated Metabolites") +
  scale_color_manual(values = c("No Simulated Metabolites" = "blue", "Including Simulated Metabolites" = "red")) +
  theme_minimal() +
  theme(plot.title = element_text(size = 14), legend.title = element_text(size = 12), legend.text = element_text(size = 10))

# Save the plot to a file
ggsave(filename = "elastic_net_auc_plot.png", plot = aucplot_en, width = 8, height = 6, dpi = 300)
